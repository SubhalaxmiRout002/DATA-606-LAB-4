...
-   Note that normal probability plots C and D have a slight stepwise pattern.  
    ...
    ```
    In plots C and D we have a slight  stepwise patterns because age has discreate nature of data. 
    Due to numeric data type  we see the steps as jump from one value to another. And there may be 
    the repeat of records means more than 1 recors has same value.
...
-   As you can see, normal probability plots can be used both to assess 
    ...
    ```
    From QQ plot, shows unimodal right-skewed because the data diverges from the normal line     
    at the right tail of the plot. We can see the same result in the histogram.
    ```
